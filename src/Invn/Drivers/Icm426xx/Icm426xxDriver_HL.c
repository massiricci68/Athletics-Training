...
int inv_icm426xx_device_reset(struct inv_icm426xx *s)
{
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG4_B1, 1,
	                                &intf_cfg4_reg); // AUX SPI and AP SPI fields
	...
	if (ICM426XX_PWR_MGMT_0_ACCEL_MODE_LP != accel_mode) {
		...
		if (s->wu_off_acc_odr_changes >= 8) {
			...
			new_freq = (cur_freq != ICM426XX_ACCEL_CONFIG0_ODR_1_5625_HZ) ?
                           ...
                           ICM426XX_ACCEL_CONFIG0_ODR_3_125_HZ;
...
int inv_icm426xx_disable_accel(struct inv_icm426xx *s)
{
	...
	if (stop_fifo_usage && s->fifo_is_used) {
		status |= inv_icm426xx_read_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);
		data &= (uint8_t) ~(BIT_FIFO_CONFIG1_ACCEL_MASK | BIT_FIFO_CONFIG1_GYRO_MASK |
		                    BIT_FIFO_CONFIG1_HIRES_MASK);
		status |= inv_icm426xx_write_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);

		/* Reset FIFO explicitly so the new configuration is taken into account */
                                ...
                                inv_icm426xx_interrupt_parameter_t *interrupt_to_configure)
{
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE8_B4, 3,
	                                data); /* lettura a raffica int_source8/int_source9/int_source10 */
	status |= inv_icm426xx_set_reg_bank(s, 0);

	/* Handles INT_SOURCE8 bits */
	interrupt_to_configure->INV_ICM426XX_UI_FSYNC = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE8_UI_FSYNC_IBI_EN) >> BIT_INT_UI_FSYNC_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_UI_DRDY = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE8_UI_DRDY_IBI_EN) >> BIT_INT_UI_DRDY_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_FIFO_THS = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE8_FIFO_THS_IBI_EN) >> BIT_INT_FIFO_THS_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_FIFO_FULL = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE8_FIFO_FULL_IBI_EN) >> BIT_INT_FIFO_FULL_IBI_EN_POS);

	/* Handles INT_SOURCE9 bits */
	interrupt_to_configure->INV_ICM426XX_SMD = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE9_SMD_IBI_EN) >> BIT_INT_SMD_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_X = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE9_WOM_X_IBI_EN) >> BIT_INT_WOM_X_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_Y = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE9_WOM_Y_IBI_EN) >> BIT_INT_WOM_Y_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_Z = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE9_WOM_Z_IBI_EN) >> BIT_INT_WOM_Z_IBI_EN_POS);

	/* Handles INT_SOURCE10 bits */
	interrupt_to_configure->INV_ICM426XX_STEP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE10_STEP_DET_IBI_EN) >> BIT_INT_STEP_DET_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_STEP_CNT_OVFL = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE10_STEP_CNT_OVFL_IBI_EN) >> BIT_INT_STEP_CNT_OVFL_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_TILT_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE10_TILT_DET_IBI_EN) >> BIT_INT_TILT_DET_IBI_EN_POS);
	interrupt_to_configure->INV_ICM426XX_TAP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE10_TAP_DET_IBI_EN) >> BIT_INT_TAP_DET_IBI_EN_POS);
                                 ...
                                 inv_icm426xx_interrupt_parameter_t *interrupt_to_configure)
{
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE0, 2,
	                                data); /* burst read int_source0/int_source1 */
	status |= inv_icm426xx_set_reg_bank(s, 4);
	...
	interrupt_to_configure->INV_ICM426XX_UI_FSYNC = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE0_UI_FSYNC_INT1_EN) >> BIT_INT_UI_FSYNC_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_UI_DRDY = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE0_UI_DRDY_INT1_EN) >> BIT_INT_UI_DRDY_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_FIFO_THS = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE0_FIFO_THS_INT1_EN) >> BIT_INT_FIFO_THS_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_FIFO_FULL = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE0_FIFO_FULL_INT1_EN) >> BIT_INT_FIFO_FULL_INT_EN_POS);

	/* Handles INT_SOURCE1 bits */
	interrupt_to_configure->INV_ICM426XX_SMD = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE1_SMD_INT1_EN) >> BIT_INT_SMD_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_X = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE1_WOM_X_INT1_EN) >> BIT_INT_WOM_X_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_Y = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE1_WOM_Y_INT1_EN) >> BIT_INT_WOM_Y_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_Z = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE1_WOM_Z_INT1_EN) >> BIT_INT_WOM_Z_INT_EN_POS);

	/* Handles INT_SOURCE6 bits */
	interrupt_to_configure->INV_ICM426XX_STEP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_STEP_DET_INT1_EN) >> BIT_INT_STEP_DET_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_STEP_CNT_OVFL = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_STEP_CNT_OVFL_INT1_EN) >> BIT_INT_STEP_CNT_OVFL_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_TILT_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_TILT_DET_INT1_EN) >> BIT_INT_TILT_DET_INT_EN_POS);
#if defined(ICM_FAMILY_BPLUS)
	interrupt_to_configure->INV_ICM426XX_SLEEP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_SLEEP_DET_INT1_EN) >> BIT_INT_SLEEP_DET_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WAKE_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_WAKE_DET_INT1_EN) >> BIT_INT_WAKE_DET_INT_EN_POS);
#elif defined(ICM_FAMILY_CPLUS)
	interrupt_to_configure->INV_ICM426XX_FF_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_FF_DET_INT1_EN) >> BIT_INT_FF_DET_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_LOWG_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_LOWG_DET_INT1_EN) >> BIT_INT_LOWG_DET_INT_EN_POS);
#endif
	interrupt_to_configure->INV_ICM426XX_TAP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE6_TAP_DET_INT1_EN) >> BIT_INT_TAP_DET_INT_EN_POS);
                                 ...
                                 inv_icm426xx_interrupt_parameter_t *interrupt_to_configure)
{
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE3, 2,
	                                data); /* burst read int_source3/int_source4 */
	status |= inv_icm426xx_set_reg_bank(s, 4);
	...
	interrupt_to_configure->INV_ICM426XX_UI_FSYNC = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE3_UI_FSYNC_INT2_EN) >> BIT_INT_UI_FSYNC_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_UI_DRDY = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE3_UI_DRDY_INT2_EN) >> BIT_INT_UI_DRDY_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_FIFO_THS = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE3_FIFO_THS_INT2_EN) >> BIT_INT_FIFO_THS_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_FIFO_FULL = (inv_icm426xx_interrupt_value)(
	    (data[0] & BIT_INT_SOURCE3_FIFO_FULL_INT2_EN) >> BIT_INT_FIFO_FULL_INT_EN_POS);

	/* Handles INT_SOURCE4 bits */
	interrupt_to_configure->INV_ICM426XX_SMD = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE4_SMD_INT2_EN) >> BIT_INT_SMD_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_X = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE4_WOM_X_INT2_EN) >> BIT_INT_WOM_X_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_Y = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE4_WOM_Y_INT2_EN) >> BIT_INT_WOM_Y_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WOM_Z = (inv_icm426xx_interrupt_value)(
	    (data[1] & BIT_INT_SOURCE4_WOM_Z_INT2_EN) >> BIT_INT_WOM_Z_INT_EN_POS);

	/* Handles INT_SOURCE7 bits */
	interrupt_to_configure->INV_ICM426XX_STEP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_STEP_DET_INT2_EN) >> BIT_INT_STEP_DET_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_STEP_CNT_OVFL = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_STEP_CNT_OVFL_INT2_EN) >> BIT_INT_STEP_CNT_OVFL_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_TILT_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_TILT_DET_INT2_EN) >> BIT_INT_TILT_DET_INT_EN_POS);
#if defined(ICM_FAMILY_BPLUS)
	interrupt_to_configure->INV_ICM426XX_SLEEP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_SLEEP_DET_INT2_EN) >> BIT_INT_SLEEP_DET_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_WAKE_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_WAKE_DET_INT2_EN) >> BIT_INT_WAKE_DET_INT_EN_POS);
#elif defined(ICM_FAMILY_CPLUS)
	interrupt_to_configure->INV_ICM426XX_FF_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_FF_DET_INT2_EN) >> BIT_INT_FF_DET_INT_EN_POS);
	interrupt_to_configure->INV_ICM426XX_LOWG_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_LOWG_DET_INT2_EN) >> BIT_INT_LOWG_DET_INT_EN_POS);
#endif
	interrupt_to_configure->INV_ICM426XX_TAP_DET = (inv_icm426xx_interrupt_value)(
	    (data[2] & BIT_INT_SOURCE7_TAP_DET_INT2_EN) >> BIT_INT_TAP_DET_INT_EN_POS);
                                ...
                                inv_icm426xx_interrupt_parameter_t *interrupt_to_configure)
{
	...
	status |= inv_icm426xx_set_reg_bank(s, 4);
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE8_B4, 3,
	                                data); /* burst read int_source8/int_source9/int_source10 */

	/* Set INT_SOURCE8 bits */
	data[0] &= (uint8_t) ~(BIT_INT_SOURCE8_UI_FSYNC_IBI_EN | BIT_INT_SOURCE8_UI_DRDY_IBI_EN |
	                       BIT_INT_SOURCE8_FIFO_THS_IBI_EN | BIT_INT_SOURCE8_FIFO_FULL_IBI_EN);
	data[0] |=
	    ((interrupt_to_configure->INV_ICM426XX_UI_FSYNC != 0) << BIT_INT_UI_FSYNC_IBI_EN_POS);
	data[0] |= ((interrupt_to_configure->INV_ICM426XX_UI_DRDY != 0) << BIT_INT_UI_DRDY_IBI_EN_POS);
	data[0] |=
	    ((interrupt_to_configure->INV_ICM426XX_FIFO_THS != 0) << BIT_INT_FIFO_THS_IBI_EN_POS);
	data[0] |=
	    ((interrupt_to_configure->INV_ICM426XX_FIFO_FULL != 0) << BIT_INT_FIFO_FULL_IBI_EN_POS);

	/* Set INT_SOURCE9 bits */
	data[1] &= (uint8_t) ~(BIT_INT_SOURCE9_SMD_IBI_EN | BIT_INT_SOURCE9_WOM_X_IBI_EN |
	                       BIT_INT_SOURCE9_WOM_Y_IBI_EN | BIT_INT_SOURCE9_WOM_Z_IBI_EN);
	...
	data[2] &=
	    (uint8_t) ~(BIT_INT_SOURCE10_STEP_DET_IBI_EN | BIT_INT_SOURCE10_STEP_CNT_OVFL_IBI_EN |
	                BIT_INT_SOURCE10_TILT_DET_IBI_EN | BIT_INT_SOURCE10_TAP_DET_IBI_EN);
	data[2] |=
	    ((interrupt_to_configure->INV_ICM426XX_STEP_DET != 0) << BIT_INT_STEP_DET_IBI_EN_POS);
	data[2] |= ((interrupt_to_configure->INV_ICM426XX_STEP_CNT_OVFL != 0)
	            << BIT_INT_STEP_CNT_OVFL_IBI_EN_POS);
	data[2] |=
	    ((interrupt_to_configure->INV_ICM426XX_TILT_DET != 0) << BIT_INT_TILT_DET_IBI_EN_POS);
	data[2] |= ((interrupt_to_configure->INV_ICM426XX_TAP_DET != 0) << BIT_INT_TAP_DET_IBI_EN_POS);

	status |= inv_icm426xx_write_reg(s, MPUREG_INT_SOURCE8_B4, 3,
	                                 data); /* burst write int_source8/int_source9/int_source10 */
	status |= inv_icm426xx_set_reg_bank(s, 0);
                                 ...
                                 inv_icm426xx_interrupt_parameter_t *interrupt_to_configure)
{
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE0, 2,
	                                data); /* lettura a raffica int_source0/int_source1 */
	status |= inv_icm426xx_set_reg_bank(s, 4);
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE6_B4, 1,
	                                &data[2]); /* passa a bank4 per int_source6 */

	/* Set INT_SOURCE0 bits */
	data[0] &= (uint8_t) ~(BIT_INT_SOURCE0_UI_FSYNC_INT1_EN | BIT_INT_SOURCE0_UI_DRDY_INT1_EN |
	                       BIT_INT_SOURCE0_FIFO_THS_INT1_EN | BIT_INT_SOURCE0_FIFO_FULL_INT1_EN);
	data[0] |=
	    ((interrupt_to_configure->INV_ICM426XX_UI_FSYNC != 0) << BIT_INT_UI_FSYNC_INT_EN_POS);
	data[0] |= ((interrupt_to_configure->INV_ICM426XX_UI_DRDY != 0) << BIT_INT_UI_DRDY_INT_EN_POS);
	data[0] |=
	    ((interrupt_to_configure->INV_ICM426XX_FIFO_THS != 0) << BIT_INT_FIFO_THS_INT_EN_POS);
	data[0] |=
	    ((interrupt_to_configure->INV_ICM426XX_FIFO_FULL != 0) << BIT_INT_FIFO_FULL_INT_EN_POS);

	/* Set INT_SOURCE1 bits */
	data[1] &= (uint8_t) ~(BIT_INT_SOURCE1_SMD_INT1_EN | BIT_INT_SOURCE1_WOM_X_INT1_EN |
	                       BIT_INT_SOURCE1_WOM_Y_INT1_EN | BIT_INT_SOURCE1_WOM_Z_INT1_EN);
	...
	data[2] &= (uint8_t) ~(BIT_INT_SOURCE6_STEP_DET_INT1_EN |
	                       BIT_INT_SOURCE6_STEP_CNT_OVFL_INT1_EN | BIT_INT_SOURCE6_TILT_DET_INT1_EN
#if defined(ICM_FAMILY_BPLUS)
	...
	data[2] |=
	    ((interrupt_to_configure->INV_ICM426XX_STEP_DET != 0) << BIT_INT_STEP_DET_INT_EN_POS);
	data[2] |= ((interrupt_to_configure->INV_ICM426XX_STEP_CNT_OVFL != 0)
	            << BIT_INT_STEP_CNT_OVFL_INT_EN_POS);
	data[2] |=
	    ((interrupt_to_configure->INV_ICM426XX_TILT_DET != 0) << BIT_INT_TILT_DET_INT_EN_POS);
#if defined(ICM_FAMILY_BPLUS)
	data[2] |=
	    ((interrupt_to_configure->INV_ICM426XX_SLEEP_DET != 0) << BIT_INT_SLEEP_DET_INT_EN_POS);
	data[2] |=
	    ((interrupt_to_configure->INV_ICM426XX_WAKE_DET != 0) << BIT_INT_WAKE_DET_INT_EN_POS);
#elif defined(ICM_FAMILY_CPLUS)
	data[2] |= ((interrupt_to_configure->INV_ICM426XX_FF_DET != 0) << BIT_INT_FF_DET_INT_EN_POS);
	data[2] |=
	    ((interrupt_to_configure->INV_ICM426XX_LOWG_DET != 0) << BIT_INT_LOWG_DET_INT_EN_POS);
#endif
	data[2] |= ((interrupt_to_configure->INV_ICM426XX_TAP_DET != 0) << BIT_INT_TAP_DET_INT_EN_POS);

	status |=
	    inv_icm426xx_write_reg(s, MPUREG_INT_SOURCE6_B4, 1,
	                           &data[2]); /* start with int_source6 since we are still in bank4 */
	status |= inv_icm426xx_set_reg_bank(s, 0);
	status |= inv_icm426xx_write_reg(s, MPUREG_INT_SOURCE0, 2,
	                                 data); /* burst write int_source0/int_source1 */
                                 ...
                                 inv_icm426xx_interrupt_parameter_t *interrupt_to_configure)
{
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE3, 2,
	                                data); /* burst read int_source3/int_source4 */
	status |= inv_icm426xx_set_reg_bank(s, 4);
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE7_B4, 1,
	                                &data[2]); /* switch to bank4 for int_source7 */

	/* Set INT_SOURCE3 bits */
	data[0] &= (uint8_t) ~(BIT_INT_SOURCE3_UI_FSYNC_INT2_EN | BIT_INT_SOURCE3_UI_DRDY_INT2_EN |
	                       BIT_INT_SOURCE3_FIFO_THS_INT2_EN | BIT_INT_SOURCE3_FIFO_FULL_INT2_EN);
	data[0] |=
	    ((interrupt_to_configure->INV_ICM426XX_UI_FSYNC != 0) << BIT_INT_UI_FSYNC_INT_EN_POS);
	data[0] |= ((interrupt_to_configure->INV_ICM426XX_UI_DRDY != 0) << BIT_INT_UI_DRDY_INT_EN_POS);
	data[0] |=
	...
	data[1] &= (uint8_t) ~(BIT_INT_SOURCE4_SMD_INT2_EN | BIT_INT_SOURCE4_WOM_X_INT2_EN |
	                       BIT_INT_SOURCE4_WOM_Y_INT2_EN | BIT_INT_SOURCE4_WOM_Z_INT2_EN);
	data[1] |= ((interrupt_to_configure->INV_ICM426XX_SMD != 0) << BIT_INT_SMD_INT_EN_POS);
	data[1] |= ((interrupt_to_configure->INV_ICM426XX_WOM_X != 0) << BIT_INT_WOM_X_INT_EN_POS);
	data[1] |= ((interrupt_to_configure->INV_ICM426XX_WOM_Y != 0) << BIT_INT_WOM_Y_INT_EN_POS);
	data[1] |= ((interrupt_to_configure->INV_ICM426XX_WOM_Z != 0) << BIT_INT_WOM_Z_INT_EN_POS);
	...
	data[2] &= (uint8_t) ~(BIT_INT_SOURCE7_STEP_DET_INT2_EN |
	                       BIT_INT_SOURCE7_STEP_CNT_OVFL_INT2_EN | BIT_INT_SOURCE7_TILT_DET_INT2_EN
#if defined(ICM_FAMILY_BPLUS)
	...
	status |=
	    inv_icm426xx_write_reg(s, MPUREG_INT_SOURCE7_B4, 1,
	                           &data[2]); /* start with int_source7 since we are still in bank4 */
	status |= inv_icm426xx_set_reg_bank(s, 0);
	status |= inv_icm426xx_write_reg(s, MPUREG_INT_SOURCE3, 2,
	                                 data); /* burst write int_source3/int_source4 */
...
int inv_icm426xx_get_data_from_registers(struct inv_icm426xx *s)
{
	...
	/* Assicurarsi che il bit di stato dei dati pronti sia impostato */
...
/* Interruzioni del dispositivo ritardate durante la comunicazione con altri slave collegati allo stesso bus 
		 * Semi-scrittura per rilasciare l'interrupt in I2C
		 */
		...
		/* richiamata dell'evento del sensore di chiamata */
		...
		if (packet_count > 0) {
			...
			for (packet_count_i = 0; packet_count_i < packet_count; packet_count_i++) {
				...
				if (header->bits.accel_bit) {
					inv_icm426xx_format_data(s->endianess_data, &s->fifo_data[0 + fifo_idx],
					                         (uint16_t *)&event.accel[0]);
					inv_icm426xx_format_data(s->endianess_data, &s->fifo_data[2 + fifo_idx],
					                         (uint16_t *)&event.accel[1]);
					inv_icm426xx_format_data(s->endianess_data, &s->fifo_data[4 + fifo_idx],
				...
				if ((header->bits.timestamp_bit) || (header->bits.fsync_bit)) {
					...
					/* nuovo evento fsync */
/* Il primo evento FSYNC dopo l'abilitazione è irrilevante
 * Il tag FSYNC e i dati FSYNC devono essere ignorati al primo ODR dopo il riavvio.
 */
					...
					if ((event.accel[0] != INVALID_VALUE_FIFO) &&
						...
						if (s->accel_start_time_us == UINT32_MAX) {
							...
							if (!header->bits.fsync_bit) {
								 ...
								 */
								if ((inv_icm426xx_get_time_us() - s->accel_start_time_us) >=
								    ICM426XX_ACC_STARTUP_TIME_US) {
									s->accel_start_time_us = UINT32_MAX;
									event.sensor_mask |= (1 << INV_ICM426XX_SENSOR_ACCEL);
								}
							}
					...
					if ((event.gyro[0] != INVALID_VALUE_FIFO) &&
						...
						if (s->gyro_start_time_us == UINT32_MAX) {
							...
							if (!header->bits.fsync_bit) {
								 ...
								 * Compare start time with current time to notify the event
								 */
								if ((inv_icm426xx_get_time_us() - s->gyro_start_time_us) >=
								    ICM426XX_GYR_STARTUP_TIME_US) {
									s->gyro_start_time_us = UINT32_MAX;
									event.sensor_mask |= (1 << INV_ICM426XX_SENSOR_GYRO);
								}
							}
...
uint32_t inv_icm426xx_convert_odr_bitfield_to_us(uint32_t odr_bitfield)
{
 ...
 odr bitfield - frequency : odr ms
  ...
  (default) 7 - 200       : 5 ms
	...
	switch (odr_bitfield) {
	case ICM426XX_ACCEL_CONFIG0_ODR_32_KHZ:
		return 32;
	case ICM426XX_ACCEL_CONFIG0_ODR_16_KHZ:
		return 63;
	case ICM426XX_ACCEL_CONFIG0_ODR_8_KHZ:
		return 125;
	case ICM426XX_ACCEL_CONFIG0_ODR_4_KHZ:
		return 250;
	case ICM426XX_ACCEL_CONFIG0_ODR_2_KHZ:
		return 500;
	case ICM426XX_ACCEL_CONFIG0_ODR_1_KHZ:
		return 1000;
	case ICM426XX_ACCEL_CONFIG0_ODR_500_HZ:
		return 2000;
	case ICM426XX_ACCEL_CONFIG0_ODR_200_HZ:
		return 5000;
	case ICM426XX_ACCEL_CONFIG0_ODR_100_HZ:
		return 10000;
	case ICM426XX_ACCEL_CONFIG0_ODR_50_HZ:
		return 20000;
	case ICM426XX_ACCEL_CONFIG0_ODR_25_HZ:
		return 40000;
	case ICM426XX_ACCEL_CONFIG0_ODR_12_5_HZ:
		return 80000;
	case ICM426XX_ACCEL_CONFIG0_ODR_6_25_HZ:
		return 160000;
	case ICM426XX_ACCEL_CONFIG0_ODR_3_125_HZ:
		return 320000;
                                     ...
                                     const ICM426XX_ACCEL_CONFIG0_ODR_t frequency)
{
	...
	if (ICM426XX_PWR_MGMT_0_ACCEL_MODE_LP != accel_pwr_mode)
		...
		s->wu_off_acc_odr_changes =
		    0; /* WUOSC is on and acc is running, ODR change will be taken into account */
#endif
	status |= inv_icm426xx_read_reg(s, MPUREG_ACCEL_CONFIG0, 1, &accel_cfg_0_reg);
	accel_cfg_0_reg &= (uint8_t)~BIT_ACCEL_CONFIG0_ODR_MASK;
...
int inv_icm426xx_get_accel_fsr(struct inv_icm426xx *s, ICM426XX_ACCEL_CONFIG0_FS_SEL_t *accel_fsr_g)
{
	...
	if ((s->fifo_highres_enabled) && (s->fifo_is_used == INV_ICM426XX_FIFO_ENABLED))
		*accel_fsr_g = ACCEL_CONFIG0_FS_SEL_MAX;
		...
		status |= inv_icm426xx_read_reg(s, MPUREG_ACCEL_CONFIG0, 1, &accel_cfg_0_reg);
		*accel_fsr_g =
		    (ICM426XX_ACCEL_CONFIG0_FS_SEL_t)(accel_cfg_0_reg & BIT_ACCEL_CONFIG0_FS_SEL_MASK);
	}
...
int inv_icm426xx_get_gyro_fsr(struct inv_icm426xx *s, ICM426XX_GYRO_CONFIG0_FS_SEL_t *gyro_fsr_dps)
{
	...
	if ((s->fifo_highres_enabled) && (s->fifo_is_used == INV_ICM426XX_FIFO_ENABLED))
		*gyro_fsr_dps = GYRO_CONFIG0_FS_SEL_MAX;
		...
		status |= inv_icm426xx_read_reg(s, MPUREG_GYRO_CONFIG0, 1, &gyro_cfg_0_reg);
		*gyro_fsr_dps =
		    (ICM426XX_GYRO_CONFIG0_FS_SEL_t)(gyro_cfg_0_reg & BIT_GYRO_CONFIG0_FS_SEL_MASK);
	}
...
int inv_icm426xx_reset_fifo(struct inv_icm426xx *s)
{
	...
	if (((gyr_mode != ICM426XX_PWR_MGMT_0_GYRO_MODE_OFF) ||
	     (acc_mode != ICM426XX_PWR_MGMT_0_ACCEL_MODE_OFF)) &&
...
int inv_icm426xx_enable_timestamp_to_register(struct inv_icm426xx *s)
{
	...
	if (!s->tmst_to_reg_en_cnt) {
		/* Abilita la lettura del registro timestamp a 20 bit
 * È necessario attendere almeno 200us prima di attivare lo strobo
 */
...
int inv_icm426xx_get_current_timestamp(struct inv_icm426xx *s, uint32_t *icm_time)
{
	...
	/* Abilita il bloccaggio del contatore timestamp nel registro timestamp */
...
int inv_icm426xx_enable_clkin_rtc(struct inv_icm426xx *s, uint8_t enable)
{
	...
	if (enable) {
		/* CLKIN non è supportato per tutte le parti, in questo caso impedire l'abilitazione di CLKIN. */
#if (RTC_SUPPORTED)
...
#else
		status |= inv_icm426xx_read_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);
		data |= (BIT_FIFO_CONFIG1_GYRO_MASK | BIT_FIFO_CONFIG1_ACCEL_MASK |
		         BIT_FIFO_CONFIG1_TEMP_MASK | BIT_FIFO_CONFIG1_TMST_FSYNC_MASK);
#endif
		/* Imposta la condizione di generazione di fifo_wm_int_w: fifo_wm_int_w generato quando contatore >= soglia */
...
uint32_t inv_icm426xx_get_reg_timestamp_resolution_us_q24(struct inv_icm426xx *s)
{
	...
	/* RTC è abilitato, la risoluzione del timestamp è un tick di RTC
 * Il nostro RTC funziona a 32768 Hz, quindi la risoluzione è 1/32768 s o 1000000/32768 us
 */
...
static int inv_icm426xx_init_hardware_from_ui(struct inv_icm426xx *s)
{
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_ACCEL_WOM_X_THR_B4, sizeof(wom_threshold),
	                                 &wom_threshold[0]);

	status |= inv_icm426xx_set_reg_bank(s, 0); /* Set memory bank 0 */
	data = ((uint8_t)ICM426XX_SMD_CONFIG_WOM_INT_MODE_ANDED) |
	       ((uint8_t)ICM426XX_SMD_CONFIG_WOM_MODE_CMP_PREV);
...
static int inv_icm426xx_is_wu_osc_active(struct inv_icm426xx *s)
{
	...
	return ((gyr_mode == ICM426XX_PWR_MGMT_0_GYRO_MODE_OFF ||
	         gyr_mode == ICM426XX_PWR_MGMT_0_GYRO_MODE_STANDBY) /* Gyro is off or standby */
